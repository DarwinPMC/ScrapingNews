...
# Endpoint para obtener la cantidad de noticias por día
@app.route('/api/noticias/count-daily', methods=['GET'])
def get_count_daily():
    connection = connect_to_mysql()
    if connection:
        try:
            ...
            return jsonify(daily_counts), 200
        except Error as e:
            print(f"Error al obtener conteo diario: {e}")
            return jsonify({"error": "Error en el servidor"}), 500
        finally:
            cursor.close()
            connection.close()
...
# Endpoint para obtener la cantidad de noticias por semana
@app.route('/api/noticias/count-weekly', methods=['GET'])
def get_count_weekly():
    connection = connect_to_mysql()
    if connection:
        try:
            cursor = connection.cursor(dictionary=True)
            query = """
                SELECT YEAR(fecha) as anio, WEEK(fecha) as semana, COUNT(*) as total
                FROM noticias
            ...
            return jsonify(weekly_counts), 200
        except Error as e:
            print(f"Error al obtener conteo semanal: {e}")
            return jsonify({"error": "Error en el servidor"}), 500
        finally:
            cursor.close()
            connection.close()
...
# Endpoint para obtener la cantidad de noticias por mes
@app.route('/api/noticias/count-monthly', methods=['GET'])
def get_count_monthly():
    connection = connect_to_mysql()
    if connection:
        try:
            cursor = connection.cursor(dictionary=True)
            query = """
                SELECT YEAR(fecha) as anio, MONTH(fecha) as mes, COUNT(*) as total
                FROM noticias
            ...
            print(f"Error al obtener conteo mensual: {e}")
            return jsonify({"error": "Error en el servidor"}), 500
        finally:
            cursor.close()
            connection.close()
